...
\begin{frame}
	...
	\begin{minipage}[t]{0.45\textwidth}
		...
		\begin{tikzpicture}
			...
			\Vertex[x=-1, y=-1]{level_l}
			\Vertex[x=-1, y=3]{level_u}
			\Edge[Direct, label=level, position=left, lw=1pt](level_l)(level_u)
...
\begin{frame}
	...
	\begin{minipage}[t]{0.45\textwidth}
		...
		\begin{tikzpicture}
			...
			\Vertex[x=0, y=0]{origin}

			%x-axis
			\Vertex[x=3, y=0]{x_u}
			\Edge[Direct, label=x, position=below, lw=1pt](origin)(x_u)

			%y-axis
			\Vertex[x=0, y=3]{y_u}
			\Edge[Direct, label=y, position=left, lw=1pt](origin)(y_u)

			\tikzset{VertexStyle/.style =
					{shape=circle, fill=black, minimum size = 4pt,inner sep=0pt}
			}
			\Vertex[x=2,y=4, label=1]{1}
			\Vertex[x=3,y=2, label=2]{2}
			\Vertex[x=4,y=4, label=3]{3}
			\Vertex[x=6,y=5, label=4]{4}
			\Vertex[x=7,y=7, label=5]{5}
			\Vertex[x=9,y=6, label=6]{6}
			\Vertex[x=5,y=8, label=7]{7}
			\Vertex[x=3,y=8, label=8]{8}
			...
			%t_1
			\draw
			(1) edge[auto shift, color=red] node {$t_1$} (3)
			...
			%t_2
			\draw
			(2) edge[auto shift, color=ForestGreen] node {$t_2$} (3)
	...
	\begin{minipage}[t]{0.45\textwidth}
		\vspace{0pt}
		\begin{itemize}
			\item<1-> Inverted Index: Associate a trajectory with all edges of its short representation in $E^+$.
		\end{itemize}
	\end{minipage}
\end{frame}
...
\begin{frame}
	...
	\begin{itemize}
		\item<1-> Efficient parallelization possible by identifying a tree within the DAG
